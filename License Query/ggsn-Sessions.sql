...
SELECT node_hostname,  ROUND((`Current GTPC Sessions ARG`)) as 'Current GTPC Sessions ARG' FROM (
...
SELECT node_hostname,  ROUND((`Current GTPC Sessions PY`)) as 'Current GTPC Sessions PY' FROM (
SELECT timestamp(EMS_GTPC2.`localdate`,EMS_GTPC2.`localtime`) as "timestamp",EMS_GTPC2.node_hostname, EMS_GTPC2.servname, 
...
SELECT node_hostname,  ROUND((`Current GTPC Sessions UY`)) as 'Current GTPC Sessions UY' FROM (
SELECT timestamp(EMS_GTPC2.`localdate`,EMS_GTPC2.`localtime`) as "timestamp",EMS_GTPC2.node_hostname, EMS_GTPC2.servname, 

...
SELECT pgwAR.node_hostname, ROUND(MAX(`Current PGW Sessions`)) as 'Current PGW Sessions ARG' FROM (
...
SELECT pgwPY.node_hostname, ROUND(MAX(`Current PGW Sessions`)) as 'Current PGW Sessions PY' FROM (
SELECT timestamp(EMS_PGW1.`localdate`,EMS_PGW1.`localtime`) as 'timestamp',EMS_PGW1.node_hostname, EMS_PGW1.servname, 
EMS_PGW1.`sess-cur` as 'Current PGW Sessions' 
FROM mos.EMS_PGW1 
...
SELECT pgwUY.node_hostname, ROUND(MAX(`Current PGW Sessions`)) as 'Current PGW Sessions UY' FROM (
SELECT timestamp(EMS_PGW1.`localdate`,EMS_PGW1.`localtime`) as 'timestamp',EMS_PGW1.node_hostname, EMS_PGW1.servname, 
EMS_PGW1.`sess-cur` as 'Current PGW Sessions' 
FROM mos.EMS_PGW1 

...
height=191
thresshold=70
outputFile=PortUtilizationRx.png
...
SELECT timestamp(EMS_Port.`localdate`,EMS_Port.`localtime`) as 'timestamp', EMS_Port.node_hostname, ROUND(AVG(EMS_Port.`util-tx-curr`/10000)*100,2) as 'Port Utilization Transmit %' \
FROM mos.EMS_Port \
...
SELECT timestamp(EMS_Port.`localdate`,EMS_Port.`localtime`) as 'timestamp', EMS_Port.node_hostname, ROUND(AVG(EMS_Port.`util-tx-curr`/40000)*100,2) as 'Port Utilization Transmit %' \
FROM mos.EMS_Port \
